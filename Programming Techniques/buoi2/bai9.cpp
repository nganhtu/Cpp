...
double prep[NUM_ITER], sigm[NUM_ITER];
const double BOTTOM = -15, TOP = 15;
const double DELTA = (TOP - BOTTOM) / NUM_ITER;
...
void precalc()
{
    ...
    {
        prep[i] = BOTTOM + i * DELTA;
        sigm[i] = sigmoid_slow(prep[i]);
    }
}
...
{
    int i = floor((x - BOTTOM) / DELTA);
    return sigm[i] + (sigm[i + 1] - sigm[i]) / DELTA * (x - prep[i]);
}
